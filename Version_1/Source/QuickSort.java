import java.util.Vector;
 ...
 class QuickSort {
    ...
    ***********************************************************************/
    ...
    public static void quicksortVectorDouble(Vector v, int left, int right, int index) {
        if (right <= left) return;
    ...
    public static void quicksortVectorInteger(Vector v, int left, int right, int index) {
        if (right <= left) return;
    ...
    private static int partitionString(Vector v, int left, int right) {
        ...
        int j = right;
        while (true) {
            while (lessString((String) v.elementAt(++i), (String) v.elementAt(right)))      // find item on left to swap
                ;                               // a[right] acts as sentinel
            while (lessString((String) v.elementAt(right), (String) v.elementAt(--j)))      // find item on right to swap
                if (j == left) break;           // don't go out-of-bounds
            if (i >= j) break;                  // check if pointers cross
    ...
    private static int partitionVectorDouble(Vector v, int left, int right, int index) {
        int i = left - 1;
        int j = right;
        while (true) {
            while (lessVectorDouble((Vector) v.elementAt(++i), (Vector) v.elementAt(right), index))      // find item on left to swap
                ;                               // a[right] acts as sentinel
            while (lessVectorDouble((Vector) v.elementAt(right), (Vector) v.elementAt(--j), index))      // find item on right to swap
                if (j == left) break;           // don't go out-of-bounds
            if (i >= j) break;                  // check if pointers cross
   ...
   private static int partitionVectorInteger(Vector v, int left, int right, int index) {
        int i = left - 1;
        int j = right;
        while (true) {
            while (lessVectorInteger((Vector) v.elementAt(++i), (Vector) v.elementAt(right), index))      // find item on left to swap
                ;                               // a[right] acts as sentinel
            while (lessVectorInteger((Vector) v.elementAt(right), (Vector) v.elementAt(--j), index))      // find item on right to swap
                if (j == left) break;           // don't go out-of-bounds
            if (i >= j) break;                  // check if pointers cross
            exchVector(v, i, j);                      // swap two elements into place
        }
