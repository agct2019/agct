class JDSScaling implements Debuggable{
    ...
    public static String divergences[] = {"L22",
					  "Kullback-Leibler",
					  "Itakura-Saito",
					  "Amari (alpha = 0)"};
	...
	int i, j;
	memoryLine = new double[M.dimX];
	memoryColumn = new double[M.dimY];
	boolean binff = false;
	boolean bsupf = false;
	...
	for (i=0;i<M.dimX;i++)
	    for (j=0;j<M.dimY;j++){
		memoryLine[i] += M.coordinates[i][j];
		memoryColumn[j] += M.coordinates[i][j];
		memoryTot += M.coordinates[i][j];
	    }

	for (i=0;i<M.dimX;i++)
	    if ( (memoryLine[i] > 0.0) && ( (binf > 1.0/memoryLine[i]) || (!binff) ) ){
		binf = 1.0/memoryLine[i];
		binff = true;
	    }

	for (j=0;j<M.dimY;j++)
	    if ( (memoryColumn[j] > 0.0) && ( (bsup < 1.0/memoryColumn[j]) || (!bsupf) ) ){
	...
	for (i=0;i<memoryLine.length;i++)
	    val += (memoryLine[i]*Distortion.gradientX(this,(kappa * memoryLine[i])));

	for (j=0;j<memoryColumn.length;j++)
	    val += (memoryColumn[j]*Distortion.gradientX(this,(kappa * memoryColumn[j])));
	...
	do{
	    ...
	    //System.out.println("bi = " + bi + ", med = " + med + ", bs = " + bs + ", val = " + val);
