...
class Cluster {
    ...
    public Cluster(Clustering cc, int ng) {
        Gene gg = cc.getRightGeneSelectedGeneNumberToGeneNumber(ng, cc.myClusteringAlgorithm.getBefore());
        allGenes = new Vector();
        allGenes.add(new Integer(ng));
        center = new Pnt(cc.getRightPoint(ng, cc.myClusteringAlgorithm.getBefore()));
    ...
    public Cluster(Cluster cop) {
        ...
        manifold_Pnt3D = new Pnt3D(cop.manifold_Pnt3D.coordinates[0], cop.manifold_Pnt3D.coordinates[1], cop.manifold_Pnt3D.coordinates[2]);
        pca_Pnt3D = new Pnt3D(cop.pca_Pnt3D.coordinates[0], cop.pca_Pnt3D.coordinates[1], cop.pca_Pnt3D.coordinates[2]);
    ...
    /**
     * a,b をそれぞれnumber of geneとして,
     * centerの2乗距離 * a * b / (a + b) を返す.
     *
     * @param cc
     * @return
     */
    ...
    public double distance_Single_Linkage(Clustering cl, Cluster cc) {
        ...
        for (int i = 0; i < allGenes.size(); i++) {
            Pnt mp = cl.getRightPoint(((Integer) allGenes.elementAt(i)).intValue(), cl.myClusteringAlgorithm.getBefore());
            for (int j = 0; j < cc.allGenes.size(); j++) {
                Pnt hp = cl.getRightPoint(((Integer) cc.allGenes.elementAt(j)).intValue(), cl.myClusteringAlgorithm.getBefore());
...
public class AGCTClustering_HC extends AGCTClustering_Algorithm {
    ...
    public static double getDistance(AGCTClustering_HC ref, Cluster c1, Cluster c2) {
        ...
        if (ref.distanceType == 0)
            ...
            val = c1.distance_Single_Linkage(ref.getMyClustering(), c2);
...
//	}
                ...
                for (i = 0; i < nonDummyHierarchy.size(); i++) {
                    ...
                    if (vv.structPlottedCluster(cc.clusterIndex)) {
                        ...
                        if ((i1 != -1) && (i2 != -1)) {
                            ...
                            v.drawShadowLineVsRef(p, p1, JAGCTGraphicsPanel.minColor, getMyClustering().colorCluster[cc.clusterIndex], false);
                            v.drawShadowLineVsRef(p, p2, JAGCTGraphicsPanel.minColor, getMyClustering().colorCluster[cc.clusterIndex], false);
                        }
                    }
                }
    ...
    public String toString() {
        String val = "Hierarchical clustering with " + getNumberOfClusters() + " centers.\n";
    ...
    public void init_CAH() {
        ...
        for (i = 0; i < getMyClustering().getRightNumberSelectedGenes(getBefore()); i++) {
            cc = new Cluster(getMyClustering(), i);
    ...
    public void trueTops() {
        int i;
        boolean[] plotted = new boolean[getNumberOfClusters()];
        Cluster cl;
    ...
    public void CAH() {
        ...
        AGCTCounter ccc = new AGCTCounter("Clustering HC", ntot - getNumberOfClusters());
        while (nbtc > 1) {
            one_step_CAH(true);
            if (nbtc > getNumberOfClusters())
                one_step_CAH(false);
		...
		/*
         * System.out.println("Dummy Hierarchy size = " +
		 * dummyHierarchy.size()); for (i=0;i<dummyHierarchy.size();i++)
        ...
        for (i = nonDummyClusters.size() - 1; i >= 0; i--) {
            ...
            for (j = 0; j < cc.allGenes.size(); j++) {
                ...
                for (k = 0; k < getNumberOfClusters(); k++) {
                    if (k == i)
                        getSoft_memberships().set(k, id, 1.0);
                    else
                        getSoft_memberships().set(k, id, 0.0);
                }
