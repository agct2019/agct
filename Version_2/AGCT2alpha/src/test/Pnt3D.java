...
public class Pnt3D extends Pnt {
    ...
    public static Pnt3D fromSpherical(double r, double theta, double phi) {
        return new Pnt3D(r * Math.cos(theta) * Math.cos(phi), r * Math.sin(theta) * Math.cos(phi), r * Math.sin(phi));
    }

    public static Pnt3D fromCylindrical(double r, double theta, double y) {
        return new Pnt3D(r * Math.cos(theta), y, r * Math.sin(theta));
    }
    ...
    public Pnt3D cross(Pnt3D x) {
        return new Pnt3D(coordinates[1] * x.coordinates[2] - x.coordinates[1] * coordinates[2], coordinates[2] * x.coordinates[0] - x.coordinates[2] * coordinates[0], coordinates[0]
                * x.coordinates[1] - x.coordinates[0] * coordinates[1]);
    }
}
